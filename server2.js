...
app.get('/chat', function(req, res)
{
    ...
    io.sockets.on('connection', function(socket)
    {
        ...
        var requete = "SELECT ID, Participants, Name, Message FROM conversation WHERE Participants REGEXP ? OR Participants REGEXP ? ORDER BY Date";
        bdd.query(requete, ['(^' + req.session.userID + ')', '(' + req.session.userID + '$)'], function(err, result)
        {
            ...
            result.forEach(function(item)
            {
                ...
                var requetteLastMessage = "SELECT Message, Sender, Seen FROM message WHERE ID = ?";
                ...
                {
                    console.log("Discussion between " + splitted[0] + " and " + splitted[1]);
        ...
        socket.on('load-message', function(data)
        {
            ...
            {
                var ext = event.file.name.substr(event.file.name.lastIndexOf('.') + 1, event.file.name.length );
                var id = data.split(';')[0];
                var message = data.substring(data.indexOf(';') + 1, data.length);
                ...
                console.log("Send " + array.Type + " to " + id);
                ...
                bdd.query(select, [like1, like2], function(error, result, fields)
                {
                    ...
                    if(result.length > 0)
                    {
                        array.ConversationID = result[0].ID;
                        var insert = "INSERT INTO message SET ?";
                        var insertID = 0;
            ...
            bdd.query(requete, [like1, like2], function(err, result)
            {
                if(result.length > 0)
                {
                    ...
                    {
                        var selectMessage = "SELECT Sender, Message, Date FROM message WHERE ID=?";
                        bdd.query(selectMessage, [item], function(err, result)
                        {
                            if(result[0].Sender == req.session.userID)
                            {
                               result[0].Sender = 0;
                            }

                            socket.emit('load-message', JSON.stringify(result[0]));
                        });
                        ...
                        {
                            if(err) throw err;
                            console.log("Message " + item + " of user " + id + " are seen.");
                        });
                    });
                }
                else
